...
public final class HiddenLayerTrain implements Serializable {
     ...
     */
    public static void train(HiddenLayer hLayer, JavaRDD<SampleVector> sampleX, SGDTrainConfig config, boolean cache) {
    	int x_n = (int) sampleX.count();
        int nrModelReplica = config.getNbrModelReplica();
        
        // model split
        ModelReplicaSplit<SampleVector> split = new ModelReplicaSplit<SampleVector>();
        JavaPairRDD<Integer, List<SampleVector>> splitX = split.split(sampleX, nrModelReplica, cache);
